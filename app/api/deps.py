...
async def get_current_user_dep(
    ...
    """JWT-only auth. Rejects tfsk_ keys."""
    if credentials is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if token.startswith(KEY_PREFIX):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="API key provided where user token expected",
        )
...
async def get_current_user_or_project(
    credentials: HTTPAuthorizationCredentials | None = Depends(bearer_scheme),
    session: AsyncSession = Depends(get_db),
) -> tuple[User | None, AuthContext | None]:
    """Try JWT first; if it's a tfsk_ key, try project auth. Returns (user, auth_context) — one will be None."""
    if credentials is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Missing token",
    ...
    # JWT token (not starting with tfsk_)
    ...
    return None, AuthContext(project=api_key.project, api_key=api_key)